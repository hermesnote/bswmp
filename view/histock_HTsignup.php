...
$projectNO = $HTsqlFETCHcompetData[2]; //競賽代號
$projectBach = substr($HTsqlFETCHcompetData[2], 2);
$projectName = $HTsqlFETCHcompetData[3]; //競賽名稱
$start = $HTsqlFETCHcompetData[5]; //開始時間
$end = $HTsqlFETCHcompetData[6]; //結束時間

//取得場次及名額
$bach1 = substr($HTsqlFETCHcompetData[7], 0, 10);
$bach1weekday = "星期".$weekarray[date("w", strtotime($bach1))];
$bach1Limit = $HTsqlFETCHcompetData[9];
$bach2 = substr($HTsqlFETCHcompetData[10], 0, 10);
$bach2weekday = "星期".$weekarray[date("w", strtotime($bach2))];
$bach2Limit = $HTsqlFETCHcompetData[12];
$bach3 = substr($HTsqlFETCHcompetData[13], 0, 10);
$bach3weekday = "星期".$weekarray[date("w", strtotime($bach3))];
$bach3Limit = $HTsqlFETCHcompetData[15];

//取得各場次已報名
$sqlBach1Count = mysql_query("
	SELECT * FROM histock_HTsignup WHERE bach = 'bach1' AND projectNO = '$projectNO'
");
$bach1Used = mysql_num_rows($sqlBach1Count);

$sqlBach2Count = mysql_query("
	SELECT * FROM histock_HTsignup WHERE bach = 'bach2' AND projectNO = '$projectNO'
");
$bach2Used = mysql_num_rows($sqlBach2Count);

$sqlBach3Count = mysql_query("
	SELECT * FROM histock_HTsignup WHERE bach = 'bach3' AND projectNO = '$projectNO'
");
...
<div class="main">
	<div class="containerIndex">
		...
		<form class="" id="signupFormHT" method="POST" style="padding-top: 50px;">

			<div class="h3 mx-auto text-center" style="color: #0B7376;">歡迎參加</div>
			<div class="h4 mx-auto text-center" style="color: #0B7376;">第 <? echo substr($projectNO, 2); ?> 梯</div>
			<div class="h5 mx-auto text-center" style="color: #000000.;"><?php echo $projectName; ?></div>
		
				<div class="row mt-4">
					<div class="col-2"></div>
						<div class="col-4">
							<label for="disdrict">學校分區</label>
							<select name="histockDistrict" class="form-control text-center mx-auto" id="histockDistrict" style="text-align:center;text-align-last:center;">
							</select>
						</div>
						<div class="col-4">
							<label for="disdrict">所在城市</label>
							<select name="histockArea" class="form-control" id="histockArea" style="text-align:center;text-align-last:center;">
							</select>
						</div>
					<div class="col-2"></div>
				</div>
				...
				<div class="row mt-4">
					<div class="col-2"></div>
						<div class="col-6">
							...
							<select name="histockSchool" class="form-control" id="histockSchool" style="text-align:center;text-align-last:center;">
							</select>
						</div>
					<div class="col-2">
						<label for="school">稱謂</label>
						<select class="form-control" id="sex" style="text-align:center;text-align-last:center;">
							<option>女士</option>
							<option>先生</option>
						</select>
					</div>
				...
				<div class="row mt-2">
					<div class="col-2"></div>
						<div class="col-8">
							...
							<input type="text" placeholder="&nbsp;" maxlength="8" style="text-align: center" id="depart">
							<span class="label1">任教科別</span>
							<span class="border1"></span>
							</label>
							<small id="" class="form-text text-muted text-right"><span id="departMsg">※ 請填寫完整科系名稱</span></small>
						</div>
					<div class="col-2"></div>
				</div>
				...
				<div class="row mt-0">
					<div class="col-2"></div>
						<div class="col-8">
							<label for="inp" class="inp">
							<input type="text" placeholder="&nbsp;" maxlength="10" style="text-align: center" id="mobile">
				...
				<div class="row mt-2">
					...
					<div class="col-8 text-center">
						<label for="school">請選擇本次研習場次</label>
						<select class="form-control" id="bach" style="text-align:center;text-align-last:center;">
							<option value="bach1">北區：<? echo $bach1; ?> <? echo $bach1weekday; ?> (尚有名額:<? echo $bach1Left; ?>)</option>
							<option value="bach2">中區：<? echo $bach2; ?> <? echo $bach2weekday; ?> (尚有名額:<? echo $bach2Left; ?>)</option>
							<option value="bach3">南區：<? echo $bach3; ?> <? echo $bach3weekday; ?> (尚有名額:<? echo $bach3Left; ?>)</option>
						</select>
					</div>
					<div class="col-2"></div>
				</div>
				...
				<div class="row mt-3">
					<div class="col-2"></div>
						<div class="col-8">
							<label class="customcheck">為保障活動參與者之權益，請於使用本服務前，詳細閱讀本站相關活動之「活動辦法」、「競賽規則」以及「隱私權保護政策」之所有內容，當勾選此項同意時，即表示您已閱讀、瞭解並同意接受本活動之所有內容。
			...
			<button type="button" class="form-control btn btn-lg btn-outline-warning buttonFail" style="text-align: center; width: 50%" onClick="HTgetCode()">送出報名</button>
...
<input type="hidden" name="HScompet" id="HTcompet" Value="<?php echo $HTcompet; ?>">
<input type="hidden" name="HScompetprojectNO" id="HTcompetprojectNO" Value="<?php echo $projectNO; ?>">
<input type="hidden" name="HScompetprojectName" id="HTcompetprojectName" Value="<?php echo $projectName; ?>">
...
<script type="text/javascript" src="../controller/histock_HTgetCode.js"></script>
		
<!-- 自定義 js -->
<!--<script type="text/javascript" src="../controller/noMobileDevice.js"></script>-->
...
//輸入框click時, 將小字改回
$("#departMsg").click(function(){
	$("#departMsg").html("※ 請填寫完整科系名稱");
	$("#departMsg").css('color', '');
	$("#departMsg").css('font-weight', '');
});
$("#name").click(function(){
...
$("#mobileMsg").click(function(){
	$("#mobileMsg").html("※ 填寫格式：0912345678");
	$("#mobileMsg").css('color', '');
	$("#mobileMsg").css('font-weight', '');
});

...
$surveyNumber = $sqlFETCHcompetList[11]; //對應問卷
	...
	if ( $surveyNumber != '' ){
		...
		$sqlSELECTsurveyFeed = mysql_query("
			SELECT *
			FROM studentsInfo
			LEFT JOIN $surveyNumber
			ON studentsInfo.teamNO = $surveyNumber.teamNO
			AND studentsInfo.remarks = $surveyNumber.role
			WHERE $surveyNumber.role is null
...
if ($competInfo == "SG"){
	...
	//超過10隊以上修改報名費
	if ( $schoolNUMs >= 10 ){ //&& ($payWay == 'ATM轉帳' || $payWay == '超商代碼')
		$amount = $amount*0.85;
	}
...
if ($payStatus == "繳費完成"){
		...
		$receiptButton = "TRUE";
		//取得收據號碼
		$sqlSELECTreceiptNO = " SELECT * FROM receiptList WHERE orderNO = '$orderNO' ";
		$sqlRESULTreceiptNO = mysql_query($sqlSELECTreceiptNO, $sqlLink);
		$sqlFETCHreceiptNO = mysql_fetch_row($sqlRESULTreceiptNO);
		$receiptNO = $sqlFETCHreceiptNO[2];  // 收據號碼
		$receiptMN = $sqlFETCHreceiptNO[7];  // 收據金額
}else{
	$receiptButton = "FALSE";
}
...
/* 取號遮蔽 */	
.loading_popup {
  background-image: url('../img/waitLoading_01.gif');
...
<!-- 取號遮蔽 -->
<div id="loading_popup" class="loading_popup loading_hide"></div>
		...
		<div class="" style="padding-top: 30px;">
			...
			<div class="collapse" id="buttonUpload">
				<div class="form-group mx-auto tyext-center">
					<button type="button" class="mt-2 form-control btn btn-info" id="report1Button" style="text-align: center; width: 50%" onClick="uploadReport1()">初賽報告</button>
					<button type="button" class="mt-2 form-control btn btn-info" id="report2Button" style="text-align: center; width: 50%" onClick="uploadReport2()">決賽報告</button>
					<button type="button" class="mt-2 form-control btn btn-info" id="report3Button" style="text-align: center; width: 50%" onClick="uploadReport3()">決賽簡報</button>
				</div>
			</div>
...
<section class="price_area p_120" id="competCollegeInfo">
	<div class="container">
		...
		<div class="tab-pane fade" id="captainProfile-College" role="tabpanel" aria-labelledby="nav-profile-tab">
			...
			<div class="mx-auto text-center mt-5">
					<button type="button" class="btn btn-success" id="captainEditButton-College" onclick="captainEditCollege()"> 修改 </button>
					<br>
					<br>
		...
		<div class="tab-pane fade" id="member1Profile-College" role="tabpanel" aria-labelledby="nav-profile-tab">
			...
			<div class="mx-auto text-center mt-5">
					...
					<br>
					<br>
					<span id="member1Msg-College">更換<u>副手</u>前，請先<span style="color: red;font-weight: bolder">刪除</span>現有<u>副手</u>再進行新增</span>
			</div>
			</form>
		</div>
		...
		<div class="tab-pane fade" id="member2Profile-College" role="tabpanel" aria-labelledby="nav-profile-tab">
			...
			<div class="mx-auto text-center mt-5">
					...
					<br>
					<br>
					<span id="member2Msg-College">更換<u>隊員</u>前，請先<span style="color: red;font-weight: bolder">刪除</span>現有<u>隊員</u>再進行新增</span>
			</div>
			</form>
		</div>
...
<section class="testimonials_area p_120">
	<div class="container">
		teamDB = <? echo $teamDB; ?><br>
		schoolPre = <? echo $schoolPre; ?><br>
		projectNO = <? echo $projectNO; ?><br>
		schoolNUMs = <? echo $schoolNUMs; ?><br>
		amount = <? echo $amount; ?><br>
...
<div>
	...
	<input type="hidden" name="receiptMN" id="receiptMN" Value="<?php echo $receiptMN; ?>">
...
<script type="text/javascript">
	...
	function ajax_payment(payment_type, invoice_status)
	{
		...
		$("#loading_popup").removeClass("loading_hide");
		//取得繳費資訊
		...
		$.ajax({
		    ...
		    success: function (sMsg){
				...
				$("#loading_popup").addClass("loading_hide");
		    },
...
<script>
var projectNO = $("#projectNO").val();
var competType = projectNO.substring(0, 2);
...
//	$("#dataEditButton").hide();
	...
	// 共用功能按鈕
	$("#payButton").hide(); // 繳費取號 -- 隱藏
	$("#receiptButton").hide();  // 列印收據 -- 隱藏
	$("#surveyButton").hide(); // 問卷調查 -- 隱藏
	$("#uploadButton").hide(); // 檔案上傳 -- 隱藏/開放
	$("#proveButton").hide(); // 參賽證明 --隱藏	
...
//	$("#proveButton").hide(); // 參賽證明 --隱藏
	...
	if (payStatus === "繳費完成"){
		$("#payButton").hide(); // 繳費取號 -- 隱藏
		$("#report2Button").hide(); // 決賽報告上傳 -- 隱藏
		$("#report3Button").hide();  // 決賽簡報上傳 -- 隱藏
	}else{
		$("#receiptButton").hide();  // 列印收據 -- 隱藏
		$("#uploadButton").hide(); // 檔案上傳 -- 隱藏
	}
	
	// 完成檔案上傳可 填寫問卷 或 取得參賽證明
	if(competType === "SG"){
		if(report1 != "已繳交"){
			$("#surveyButton").hide(); // 問卷調查 -- 隱藏
			$("#proveButton").hide(); // 參賽證明 --隱藏
		...
		if( (report1 != "已繳交") && (surveyButton != "TRUE") ){
			...
			$("#surveyButton").hide(); // 問卷調查 -- 隱藏
			$("#proveButton").hide(); // 參賽證明 --隱藏
		}else if ((report1 === "已繳交") && (surveyButton != "TRUE")){
			// 已報告+未做完問卷
			$("#proveButton").hide(); // 參賽證明 --隱藏
		}else if ((report1 === "已繳交") && (surveyButton === "TRUE")){
			// 已交報告+已做完問卷
...
console.log('competType:'+competType+','+'payStatus:'+payStatus)	
}
...
console.log('competType:'+competType+','+'payStatus:'+payStatus+','+'report1:'+report1)	
}
...
console.log('competType:'+competType+','+'payStatus:'+payStatus+','+'report1:'+report1+','+'score1:'+score1)
}

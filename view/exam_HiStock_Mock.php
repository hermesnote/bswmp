...
$getEndTime = date("Y-m-d H:i:s", strtotime($getStartTime)+(50*60) );
...
<section class="examTitle">
	<div class="container-fluid">
		<div class="row text-center">
			<div class="col-3">
				<div class="pl-2 text-left">准考證號：<? echo $examNO; ?></div>
				<div class="pl-2 text-left">測驗卷號：模擬測驗試卷</div>
			</div>
			<div class="col-6 text-center mx-auto">
				<div class="h2">金融證券投資實務測驗（模擬試卷）</div>
				<div class="h5">距離本次測驗結束時間尚有：<span id="examTimer"></span></div>
			</div>
			<div class="col-3 text-right">
				<div class="mr-5 pr-5">測驗開始時間：<? echo $getStartTime; ?></div>
				<div class="mr-5 pr-5">測驗結束時間：<? echo $getEndTime; ?></div>
			</div>
		</div>
	</div>
</section>
	...
	<div class="container">
		...
		<div class="row mt-5">
			<div class="col-12 text-center">
				<button class="btn btn-lg btn-secondary" id="endExamButton" onClick="endExam()">結束測驗</button>
			</div>
		</div>
...
//	var ansS = parseInt(document.getElementById("myBar").getElementsByTagName("h3")[0].innerHTML);  //取得現在倒數中秒數轉為int	
	...
	$.ajax({
		...
		url:"https://wmpcca.com/bswmp/form/model/exam_HiStock_Mock_getNextQuestion.php",
		data:{
			"ansS" : ansS,
...
//	var ansS = parseInt($answerClock[0].innerHTML.split(':')[1]);  //取得現在倒數中秒數轉為int
	...
	$.ajax({
		...
		url:"https://wmpcca.com/bswmp/form/model/exam_HiStock_Mock_goResult.php",
		data:{
			"ansS" : ansS,
			"listNumber" : listNumber,
			"AQ" : AQ
...
function getExam(num){
	$.ajax({
		...
		url:"https://wmpcca.com/bswmp/form/model/exam_HiStock_Mock_resultClick.php",
		data:{
			"number" : num
		},
...
$examClock.countdown(getExamCountdown(), function(event) {
...
function endExam(){
	...
	$.ajax({
		...
		url:"https://wmpcca.com/bswmp/form/model/exam_HiStock_Mock_endExam.php",
		data:{
			"end" : end
		},

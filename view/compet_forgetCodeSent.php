...
if ( preg_match("/SG/i", $teamNO) ){
	...
	$sqlFETCHteamInfo = mysql_num_rows($sqlRESULTteamInfo);
	$DBteamNO = $sqlFETCHteamInfo[3];
	...
	$DBemail = $sqlFETCHcaptainInfo[8];
	$DBidentifyNO = $sqlFETCHcaptainInfo[5];
	...
	$DBteamNO = $sqlFETCHteamInfo[3];
	$vCode = $sqlFETCHteamInfo[9];
	
	//確認隊長資料
	$sqlSELECTcaptainInfo = " SELECT * FROM $memberDB WHERE teamNO = '$teamNO' AND remarks = '隊長' ";
	$sqlRESULTcaptainInfo = mysql_query($sqlSELECTcaptainInfo, $sqlLink);
	$sqlFETCHcaptainInfo = mysql_fetch_row($sqlRESULTcaptainInfo);
	$DBemail = $sqlFETCHcaptainInfo[9];
	$DBidentify = $sqlFETCHcaptainInfo[5];
...
if ($DBteamNO == ''){
	...
	echo "alert('此隊編查無報名資料!請重新確認!');";
	echo "history.back();";
	echo "</script>";
}else if ($email != $DBemail){
	echo "<script type='text/javascript'>";
	echo "alert('隊長E-mail資料不符!請重新確認!');";
	echo "history.back();";
	echo "</script>";
}else if ($identifyNO != $DBidentify){
	echo "<script type='text/javascript'>";
	echo "alert('隊長身份證字號不符!請重新確認!');";
	echo "history.back();";
	echo "</script>";
}else{
	//發送信件
	...
	//寄發開通碼通知Email
	require_once("../model/compet_forgetCodeMailContent.php");	
}
	...
	<div class="containerIndex">
		<form class="" method="POST" style="padding-top: 50px;">
				...
				<div class="row mt-5 mx-auto text-center">
					<div class="col-2 mx-auto text-center"></div>
						<div class="col-8">
<!--							<small id="" class="form-text text-muted mx-auto text-center"><a href="../view/compet_forgetCode.php">沒有收到驗證碼嗎?</a></small>-->
						</div>
					<div class="col-2"></div>
				</div>

...
if(true)
{
    ...
    $aOrder_Info['Items']           = array('Name' => "KEYs專業版12個月", 'Price' => (int)"$amount", 'Currency' => "元", 'Quantity' => (int) "1") ;
}
// 3.透過站內付SDK 送出請求，並取得API回傳參數
if(true)
{
    ...
    try {
        ...
        //基本參數(請依系統規劃自行調整)
        $obj->Send['ReturnURL']         = 'https://wmpcca.com/bswmp/form/ecPay/returnURL_MoneyStar.php' ;            //付款完成通知回傳的網址
        $obj->Send['MerchantTradeNo']   = $aOrder_Info['order_id'];                                 //訂單編號
        $obj->Send['MerchantTradeDate'] = date('Y/m/d H:i:s');                                      //交易時間
        $obj->Send['TotalAmount']       = $amount;                             //交易金額
        $obj->Send['TradeDesc']         = 'KEYs專業版12個月' ;   //交易描述

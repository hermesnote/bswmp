...
//隊名使用狀況
$sqlSELECTteamName = " SELECT * FROM competSocial WHERE projectNO = '$projectNO' AND teamName = '$teamName' ";
...
//隊長報名狀況
$sqlSELECTname = " SELECT * FROM socialInfo WHERE projectNO = '$projectNO' AND identifyNO = '$identifyNO' ";
...
//寫入資料庫 competSocial
$sqlINSERTData = "
	INSERT INTO competSocial ( signupTime, projectNO, teamNO, teamName, name, identifyNO, email, vCode )
	VALUES ( '$getToday', '$projectNO', '$teamNO', '$teamName', '$name', '$identifyNO', '$email', '$vCode' )
";
$sqlINSERT = mysql_query($sqlINSERTData, $sqlLink);

//寫入資料庫 socialInfo
$sqlINSERTCapData = "
	INSERT INTO socialInfo ( projectNO, teamNO, name, identifyNO, email, remarks )
	VALUES ( '$projectNO', '$teamNO', '$name', '$identifyNO', '$email', '隊長' )
";
...
//寫入Log
	...
	$log =  "\r\n$getToday $staffNO 新增了一筆 $projectNO 報名資料, 隊名$teamName, 取得隊編$teamNO, 驗證碼寄至$email";
	@fwrite($open,$log); //寫入資料
	fclose($open); //關閉檔案

//建立回傳值Array
$arr = array($teamNO, $email);

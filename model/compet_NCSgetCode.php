...
//生成隊伍編號
if ($schoolDistrict == "北區"){
	$teamNO = 'CN'.substr(date("Y"),2,2).substr(date("d"),0,2).substr(date("s"),0,2).str_pad(mt_rand(0000, 9999), 4, "0", STR_PAD_LEFT);
}
if ($schoolDistrict == "中區"){
	$teamNO = 'CC'.substr(date("Y"),2,2).substr(date("d"),0,2).substr(date("s"),0,2).str_pad(mt_rand(0000, 9999), 4, "0", STR_PAD_LEFT);
}
if ($schoolDistrict == "南區"){
	$teamNO = 'CS'.substr(date("Y"),2,2).substr(date("d"),0,2).substr(date("s"),0,2).str_pad(mt_rand(0000, 9999), 4, "0", STR_PAD_LEFT);
}
...
//寫入隊長資料庫
$sqlINSERTCapData = "
	INSERT INTO $memberDB ( registerTime, projectNO, teamNO, name, identifyNO, email, school, remarks )
